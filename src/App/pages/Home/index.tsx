  ...
  return (
    <section className="page-container row top gap-4">
      <div className="column item-2 gap-4">
        ...
        >
          {posts?.results.map((post) => (
            <Link key={post.id} className={styles.carouselLink} to={`/news/post?post_id=${post.id}`}>
              <img
                src={post.data.post_featured_image.url}
              />
              <div className="legend">
                <h4 className={styles.carouselSlideTitle}>{post.data.title[0].text}</h4>
              </div>
            </Link>
          ))}
        </Carousel>
      ...
      <div className="column item-1 gap-4">
        <Table className={styles.calendarTable} headers={["Puesto", "Piloto", "Puntos"]} elements={championship} />
